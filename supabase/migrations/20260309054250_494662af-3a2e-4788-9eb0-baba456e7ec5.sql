...
-- === REPORT_SHARES ===
-- Fix 2: Remove dead SELECT policy, keep only the functional one
DROP POLICY IF EXISTS "Deny direct SELECT access - use get_shared_report function" ON public.report_shares;
...
-- === USER_ACHIEVEMENTS ===
-- Fix 3: Already has SELECT, convert to PERMISSIVE + add explicit deny write policies
...
-- No INSERT/UPDATE/DELETE policies = default deny. Writes go through unlock_achievement() SECURITY DEFINER function.
